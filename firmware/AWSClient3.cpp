 ...
 * AWSClient3.cpp
 *
 *  See AWSClient3.h for description.
 *
 */
...
static const char* CANONICAL_FORM_POST_LINE = "PUT\n/%s\n\n";
static const int CANONICAL_FORM_POST_LINE_LEN = 7;
static const char* HTTPS_REQUEST_POST_LINE =
        "POST https://%s.%s.%s/ HTTPS/1.1\n";
static const int HTTPS_REQUEST_POST_LINE_LEN = 29;
static const char* HTTP_REQUEST_POST_LINE = "PUT /%s HTTP/1.1\n";
static const int HTTP_REQUEST_POST_LINE_LEN = 15;
...
static const char* HOST_HEADER = "host:%s.%s-%s.%s";
static const int HOST_HEADER_LEN = 8;
...
static const char* X_AMZ_CONTENT_SHA256_HEADER = "x-amz-content-sha256:%s";
static const int X_AMZ_CONTENT_SHA256_HEADER_LEN = 21;
...
static const int SIGNED_HEADERS_LEN = 64;

void AWSClient3::setKey(MinimalString key) {
    this->key = key;
}

void AWSClient3::setBucket(MinimalString bucket) {
    this->bucket = bucket;
}

MinimalString AWSClient3::getKey() const {
    return this->key;
}

MinimalString AWSClient3::getBucket() const {
    return this->bucket;
}

AWSClient3::AWSClient3() {
...
void AWSClient3::initSignedHeaders() {
    ...
    len = HOST_HEADER_LEN + bucket.length() + strlen(awsService) + strlen(awsRegion)
            + strlen(awsEndpoint);
    headers[headersCreated] = new char[len + 1]();
    sprintf(headers[headersCreated], HOST_HEADER, bucket.getCStr(), awsService, awsRegion, awsEndpoint);
    headerLens[headersCreated++] = len;

    SHA256* sha256 = new SHA256();
    char* hashed = (*sha256)(payload.getCStr(), payload.length());
    delete sha256;
    len = X_AMZ_CONTENT_SHA256_HEADER_LEN + strlen(hashed);
    headers[headersCreated] = new char[len + 1]();
    sprintf(headers[headersCreated], X_AMZ_CONTENT_SHA256_HEADER, hashed);
    headerLens[headersCreated++] = len;
...
char* AWSClient3::createStringToSign(void) {
    SHA256* sha256 = new SHA256();
    char* hashed;
    /* Calculate length of canonicalForm string. */
    int canonicalFormLen = CANONICAL_FORM_POST_LINE_LEN + key.length();
    ...
    char* postLine = new char[canonicalFormLen];
    sprintf(postLine, CANONICAL_FORM_POST_LINE, key.getCStr());

    /* Write the cannonicalForm string. */
    int canonicalFormWritten = 0;
    canonicalFormWritten += strlen(
            strcpy(canonicalForm + canonicalFormWritten, postLine));
    for (int i = 0; i < headersCreated; i++) {
...
char* AWSClient3::createSignature(const char* toSign) {
...
void AWSClient3::initUnsignedHeaders(const char* signature) {
...
void AWSClient3::createRequestInit(MinimalString &reqPayload) {
...
void AWSClient3::createRequestCleanup() {
    /* Free each header */
    for (int i = 0; i < headersCreated; i++) {
        delete[] headers[i];
    }
...
char* AWSClient3::headersToRequest() {
    /* Determine whether to use https or http postLine values. */
    int postLineLen = HTTP_REQUEST_POST_LINE_LEN;
    char* postLine = new char[postLineLen + key.length()];

    sprintf(postLine, HTTP_REQUEST_POST_LINE, key.getCStr());
...
char* AWSClient3::createRequest(MinimalString &reqPayload) {
...
char* AWSClient3::sendData(const char* data) {
